  ...
  module Parser
    class MondelezLoadXmlParserV2 < KraftLoadXmlParserV2
      ...
      def parse_shipment_header(shipment_attr_element)
        shipment_gid = shipment_attr_element.xpath("./ShipmentGid")
        gid = shipment_attr_element.children.first
        @load[1] = shipment_gid.xpath("./*[1][self::Gid]/Xid[1]").text
        @load[8] << get_domain_name(gid)
        parse_service_provider_gid_or_shipment_gid('ServiceProviderGid', shipment_attr_element.xpath("./ServiceProviderGid"))
        parse_shipment_refnum(shipment_attr_element.xpath("./ShipmentRefnum"))
        parsed_internal_shipment_status = parse_internal_shipment_status(shipment_attr_element.xpath("./InternalShipmentStatus"))
        @load[0] = 'delete' if parsed_internal_shipment_status[:status_type] == 'TRACK' && parsed_internal_shipment_status[:status_value] == 'TRACK_NO'
        @shipment_name = shipment_attr_element.xpath("./ShipmentName").text
        @load[11] = @shipment_name
      end

      # def parse_shipment_header(shipment_attr_element)
      #   shipment_attr_element.children.each do |shipment_header_attr_element|
      #     shipment_attr_element.xpath('./ShipmentGid')
